...
def test_no_token_is_anonymous():
    assert cache_identity_from_token(None) is None
    assert cache_identity_from_token("") is None


def test_identity_comes_from_the_verified_payload():
    with patch(
        "pecha_api.cache.cache_identity.validate_token",
        return_value={"sub": "user-123", "iss": "https://issuer/"},
    ):
        assert cache_identity_from_token("token") == "https://issuer/|user-123"


def test_an_unverifiable_token_is_anonymous_not_an_error():
    ...
    with patch(
        ...
        assert cache_identity_from_token("forged") is None


def test_identity_is_never_taken_without_verification():
    ...
    with patch(
        ...
        assert cache_identity_from_token("header.claims-say-victim.sig") is None
    mock_validate.assert_called_once()


def test_two_users_get_different_identities():
    def _payload(token):
        return {"sub": token, "iss": "https://issuer/"}

    with patch("pecha_api.cache.cache_identity.validate_token", side_effect=_payload):
        assert cache_identity_from_token("alice") != cache_identity_from_token("bob")
...
def test_falls_back_through_the_claims_that_identify_someone(payload, expected):
    with patch("pecha_api.cache.cache_identity.validate_token", return_value=payload):
        assert cache_identity_from_token("token") == expected
